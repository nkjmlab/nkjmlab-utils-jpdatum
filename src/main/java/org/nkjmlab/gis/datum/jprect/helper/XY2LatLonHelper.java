...
public class XY2LatLonHelper {
  ...
  public static double toLatitude(
      double x, double y, double latDegOfOrigin, double lonDegOfOrigin) {
   ...
   * @return 経度
   */
  public static double toLongitude(
      double x, double y, double latDegOfOrigin, double lonDegOfOrigin) {
    ...
    double L1 =
        ym / (prc * Math.cos(phi)) * rho
            - (Math.pow(ym, 3)
                    * (1.0 + 2.0 * Math.pow(t, 2) + Math.pow(eta, 2))
                    / (6.0 * Math.pow(prc, 3) * Math.cos(phi)))
                * rho;
    ...
    double L2 =
        (Math.pow(ym, 5)
                * (5.0
                ...
                / (120.0 * Math.pow(prc, 5) * Math.cos(phi)))
            * rho;
