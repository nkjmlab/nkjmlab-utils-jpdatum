...
import org.nkjmlab.latlngxy.util.CoordUtils;
...
public class XY2LatLng {
	...
	public static LatLng toLatLng(XY xy) {
		...
		double lat = XY2LatLng.toLat(x, y, zoneId);
		double lng = XY2LatLng.toLng(x, y, zoneId);
	...
	public static double toLat(double x, double y, int zoneId) {
		...
		return CoordUtils.degreeToDms(toLatitude(x, y, origin.getLat(),
				origin.getLng()));
	}

	public static double toLng(double x, double y, int zoneId) {
		LatLng origin = JapanPlaneRectangular.getOrigin(zoneId);
		return CoordUtils.degreeToDms(toLongitude(x, y, origin.getLat(),
				origin.getLng()));
	}

	private static double rho = 180.0 / Math.PI;

	private static double toLatitude(double x, double y, double lat, double lng) {

		double phi = calcPhi(lat, x);
		...
		double B1 = phi
				...
				* rho
				+ (Math.pow(ym, 4)
						* t
						* (5.0 + 3.0 * Math.pow(t, 2) + Math.pow(eta, 2) - 9.0
								* Math.pow(eta, 2) * Math.pow(t, 2) - 4.0 * Math
								.pow(eta, 4)) / (24.0 * mrc * Math.pow(prc, 3)))
				* rho;

		double B2 = (Math.pow(ym, 6)
				* t
				* (61.0 + 90.0 * Math.pow(t, 2) + 45.0 * Math.pow(t, 4) + 46.0
						* Math.pow(eta, 2) - 252.0 * Math.pow(t, 2)
						* Math.pow(eta, 2) - 90.0 * Math.pow(t, 4)
						* Math.pow(eta, 2)) / (720.0 * mrc * Math.pow(prc, 5)))
				* rho;
	 ...
	 * @return 経度
	 */
	private static double toLongitude(double x, double y, double lat, double lng) {
		double gentenL = AngleUtil.toAngle(lng);
		double phi = calcPhi(lat, x);
		...
		double L1 = ym
				...
				* rho
				- (Math.pow(ym, 3)
						* (1.0 + 2.0 * Math.pow(t, 2) + Math.pow(eta, 2)) / (6.0 * Math
						.pow(prc, 3) * Math.cos(phi))) * rho;

		double L2 = (Math.pow(ym, 5)
				* (5.0 + 28.0 * Math.pow(t, 2) + 24.0 * Math.pow(t, 4) + 6.0
						* Math.pow(eta, 2) + 8.0 * Math.pow(t, 2)
						* Math.pow(eta, 2)) / (120.0 * Math.pow(prc, 5) * Math
				.cos(phi))) * rho;
